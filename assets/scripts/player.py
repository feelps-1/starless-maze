...
class Player(pygame.sprite.Sprite):
    ...
    def collision(self, direction):
        ...
        for collectibles in self.collectibles_sprites:
             if collectibles.hitbox.colliderect(self.hitbox):
                if collectibles in self.collectibles_sprites:
                    if collectibles.sprite_type == "Stars":
                        self.stars += 1

                    elif collectibles.sprite_type == "Nebulae":
                        self.nebulae += 1
                        if self.health < 5:
                            self.health += 1

                    elif collectibles.sprite_type == "Bomb":
                        self.health -= 1
    ...
    def update(self):
        ...
        self.nebulaecounter.drawCounter('Nebulae', self.nebulae, 4, self.screen, (190, 25), COUNTER_BACKGROUND, (132, 0, 200))
